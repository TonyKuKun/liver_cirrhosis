...
totalseg2.py variant:
- By default, only process patient folders whose names contain "$".
- By default, overwrite/re-extract those marked patients.
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run TotalSegmentator only for $-marked patients and re-extract organ masks.",
    ...
    parser.add_argument(
        "--marker",
        default="$",
        help='Only process patient folders whose name contains this marker (default: "$")',
    )
    parser.add_argument(
        "--include-all",
        action="store_true",
        help="Process all patients instead of filtering by --marker",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="List selected patients and exit without running TotalSegmentator",
    )
    ...
    run_mode.add_argument(
        ...
        dest="overwrite",
        action="store_false",
        help="Resume interrupted runs: skip structures whose STL already exists",
    )
    run_mode.add_argument(
        "--overwrite",
        "--force",
        dest="overwrite",
        action="store_true",
        help="Overwrite and re-extract structures even if STL files already exist (default)",
    )
    ...
    if not args.include_all:
        cases = [c for c in cases if args.marker in c.name]
    if args.patient:
    ...
    scope = "all patients" if args.include_all else f"patients containing {args.marker!r}"
    print(f"[totalseg2] scope: {scope}")
    print(f"[totalseg2] {total_cases} patients, structures: {structs or 'all'}, mode: {mode}")
    if args.dry_run:
        for case in cases:
            print(f"[totalseg2] dry-run patient: {case.name}")
        return
    print(_progress_line(0, total_cases, total_start))
    for idx, case in enumerate(cases, start=1):
        case_start = time.perf_counter()
        print(f"[totalseg2] {idx}/{total_cases} {case.name}:")
        try:
        ...
        print(_progress_line(idx, total_cases, total_start))
    print(f"[totalseg2] all done in {_format_duration(time.perf_counter() - total_start)}")
