...
def mask_to_stl(mask: np.ndarray, spacing_zyx: tuple[float, float, float], out_path: str | Path) -> Path:
    ...
    write_ascii_stl(out_path, verts_xyz, faces)
    return out_path
...
def stl_to_voxels(path: str | Path, grid_size: int = 96, bounds: np.ndarray | None = None) -> tuple[np.ndarray, np.ndarray]:
    """Voxelize STL to a dense grid. Returns occupancy and xyz bounds."""
    try:
        import trimesh
    except ImportError as exc:
    ...
    points = vox.points
    idx = np.floor((points - bounds[0]) / pitch).astype(np.int64)
