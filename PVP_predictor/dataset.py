...
class PortalVeinDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Final guard: replace any remaining NaN in raw profiles (physics layer input)
        profiles_raw = torch.nan_to_num(profiles_raw, nan=0.0, posinf=0.0, neginf=0.0)
        arc_lengths = torch.nan_to_num(arc_lengths, nan=0.0, posinf=0.0, neginf=0.0)

        label = torch.tensor(d['label'], dtype=torch.float32)

        return {
            'profiles_raw': profiles_raw,       # unnormalized, for physics layer
            'profiles_norm': profiles_norm,      # normalized, for learning
    ...
    def get_norm_stats(self) -> dict:
        """Return normalization statistics for inference / saving."""
        return {
            'profile_mean': self.profile_mean,
            'profile_std': self.profile_std,
            'stat_mean': self.stat_mean,
            'stat_std': self.stat_std,
