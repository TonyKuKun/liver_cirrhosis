...
结论：当前主线代码只保留 L2 和核心合流分流 loss。旧版宽分流 loss、conductance、pressure balance、monotonic 等辅助 loss 不再进入最终训练目标。
...
4. GlobalFlowCorrector 保留；PVP 预测端收敛为单一 prediction head。
...
+ GlobalFlowCorrector
+ single PVP prediction head
+ L2/MSE + core-confluence shunt loss
- no organ boundary Q scaling
- no broad split loss by default
...
- Radiology 投稿时建议主表报告当前保留模型：`L2 + core confluence shunt loss + organ global`；历史 `L2 only` 作为消融对照。
