# PVP Predictor

This directory contains the current final portal venous pressure (PVP) regression model. The 2026-06-10 refresh keeps one PVP prediction head and only two loss terms: PVP L2/MSE plus a light core shunt-flow loss.

## Current Model

- Input: 8 vessel branches: MPV, SV, SMV, LPV, RPV, TIPS, LGV, PGV.
- Geometry: reliable cross-section features by default; noisy raw lengths and optional profile channels are excluded.
- Global patient context: spleen volume, liver volume, and spleen/liver ratio are retained.
- Prediction head: one PVP output only.
- Loss: `lambda_shunt=0.005`, `split_loss_mode=core_confluence`.
- Physics residual head: disabled by default because the 2026-06-10 ablation shows it worsens MAE.
- Training regularizer: `AuxiliaryDropoutRegularizer` is retained as a training-time stochastic regularizer, not as a second prediction head.
- Output: `oof_predictions.csv` and `oof_predictions.xlsx` use the same columns.

## Final 2026-06-10 Result

Dataset: `F:\PCG data\dataset\test4all_sample`; 17 `00`-prefix folders excluded; 72 valid samples; subject-level 5-fold; seed 40.

- Final model: MAE 2.685 +/- 0.746, RMSE 3.605, R2 0.643.
- OOF overall: MAE 2.704, RMSE 3.800, bias 0.256, n=72.
- Result directory: `runs/final_20260610_pvp_l2_shunt`.

| fold | train | val | best_epoch | MAE | RMSE | R2 |
| --- | --- | --- | --- | --- | --- | --- |
| 0 | 57 | 15 | 50 | 3.687 | 4.620 | 0.432 |
| 1 | 58 | 14 | 89 | 1.884 | 2.147 | 0.873 |
| 2 | 58 | 14 | 37 | 1.752 | 2.301 | 0.851 |
| 3 | 57 | 15 | 12 | 3.026 | 4.361 | 0.517 |
| 4 | 58 | 14 | 11 | 3.078 | 4.594 | 0.543 |

## Baseline Comparison

Traditional machine-learning baselines use the same `splits.json`. The best baseline is `physics/adaboost`: MAE 3.420, RMSE 4.286, R2 0.531.

| baseline | features | n | MAE | RMSE | R2 | bias |
| --- | --- | --- | --- | --- | --- | --- |
| physics/adaboost | 32 | 72 | 3.420 | 4.286 | 0.531 | -0.212 |
| physics/random_forest | 32 | 72 | 3.518 | 4.352 | 0.516 | -0.112 |
| physics/extra_trees | 32 | 72 | 3.532 | 4.372 | 0.512 | -0.378 |
| combined/elasticnet_cv | 1088 | 72 | 3.580 | 4.418 | 0.502 | 0.318 |
| combined/extra_trees | 1088 | 72 | 3.672 | 4.515 | 0.480 | -0.122 |
| combined/hist_gradient_boosting | 1088 | 72 | 3.677 | 4.484 | 0.487 | -0.097 |
| geometry/extra_trees | 897 | 72 | 3.685 | 4.550 | 0.472 | -0.054 |
| physics/hist_gradient_boosting | 32 | 72 | 3.699 | 4.645 | 0.449 | -0.086 |

## Ablation Summary

The final reference is `full_model`: 8-vessel layout, organ global features, GlobalFlowCorrector, FlowGraphRefiner, training dropout regularizer, no physics residual head, one PVP head, and L2 plus light core shunt loss.

| variant | category | MAE | RMSE | R2 | dMAE |
| --- | --- | --- | --- | --- | --- |
| full_model | reference | 2.685 | 3.605 | 0.643 | 0.000 |
| loss_l2_plus_core_split | loss | 2.685 | 3.605 | 0.643 | 0.000 |
| fixed_physics_params | module | 2.696 | 3.616 | 0.644 | 0.011 |
| loss_l2_only | loss | 2.700 | 3.618 | 0.641 | 0.014 |
| loss_l2_plus_full_split | loss | 2.703 | 3.619 | 0.641 | 0.018 |
| with_physics_residual | module | 2.809 | 3.837 | 0.615 | 0.123 |
| use_unreliable_raw_lengths | geometry | 2.910 | 3.977 | 0.586 | 0.224 |
| all_profile_channels | geometry | 2.930 | 4.208 | 0.542 | 0.244 |
| no_dropout_regularizer | module | 2.942 | 3.721 | 0.634 | 0.256 |
| six_vessel_layout | layout | 3.026 | 3.984 | 0.592 | 0.341 |
| no_organ_global_features | module | 3.104 | 4.093 | 0.548 | 0.419 |
| three_vessel_layout | layout | 3.132 | 4.032 | 0.580 | 0.446 |
| no_flow_graph | module | 3.171 | 4.455 | 0.490 | 0.486 |
| no_global_flow_corrector | module | 3.479 | 4.536 | 0.458 | 0.794 |

Full outputs:

- `ablation/runs/final_20260610/full/comparison.csv`
- `ablation/runs/final_20260610/full/comparison.json`
- `ablation/runs/final_20260610/full/analysis.md`

## Reproduce

Run from `PVP_predictor`:

```powershell
conda run -n pytorch python train.py ^
  --data_root "F:\PCG data\dataset\test4all_sample" ^
  --out_dir runs\final_20260610_pvp_l2_shunt ^
  --n_folds 5 --epochs 300 --seed 40 ^
  --lambda_shunt 0.005 --split_loss_mode core_confluence
```

```powershell
conda run -n pytorch python baseline/run_baselines.py ^
  --data_root "F:\PCG data\dataset\test4all_sample" ^
  --split_json runs\final_20260610_pvp_l2_shunt\splits.json ^
  --out_dir baseline\runs\final_20260610_baselines ^
  --n_points 200 --seed 40
```

```powershell
conda run -n pytorch python ablation/ablations.py ^
  --suite all --stage full ^
  --out_root ablation/runs/final_20260610 ^
  --full_n_folds 5 --full_epochs 300 --seed 40 --force
```

## Files

- `dataset.py`: vessel STL, CenterlinePoints, and organ-volume loading.
- `model.py`: current single-head PVP network.
- `train.py`: 5-fold training plus CSV/XLSX/JSON outputs.
- `baseline/`: traditional ML baselines.
- `ablation/`: module, layout, geometry, and loss ablations.
- `docs/figures/model_architecture.png`: regenerated architecture figure.
- `FINAL_20260610_RESULTS_ANALYSIS.md`: final result analysis.
