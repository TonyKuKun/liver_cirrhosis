...
SEGMENT_VESSELS_VERSION = "anatomy-anchored-v2-20260526-confluence"
...
def _select_sv_smv_pgv(branches, nodes, mpv_seg_from_lower=None):
    ...
    - Three terminal branches: SV is long and curved; PGV is the branch in
      the smaller MPV-SV wedge; SMV is finally constrained by negative-Z
      direction.
    ...
    if len(branches) >= 3:
        ...
        print(
            ...
            f"PGV endpoint={pgv_seg[-1] if pgv_seg else None}, "
...
def segment_vessels(stl_path, post_tips=None, output_json_path=None,
    ...
    if output_json_path is None:
        output_json_path = os.path.join(parentdir, "centerline_profiles_v2.json")
...
def _segment_pre_tips_no_comp(nodes, adj, endpoints, branch_points,
    ...
    return {
        ...
        'has_compensation': pgv_seg is not None,
        'compensation_type': 'PGV' if pgv_seg is not None else None,
    }
...
def _v2_pick_unused_terminal_path(candidates, used_candidates, nodes):
    ...
    for c in candidates:
        ...
        if any(_v2_same_path(c.get("branch"), used) for used in used_branches):
            continue
        options.append(c["path"])
    if not options:
        return None
    return max(options, key=lambda p: path_physical_length(p, nodes))
...
def _segment_pre_tips_with_comp(nodes, adj, endpoints, branch_points,
    ...
    if smv_candidate is None or sv_candidate is None:
        print("  V2 fallback: insufficient lower candidates, using legacy compensation logic")
        return _build_lgv_segments(
            nodes, adj, endpoints, segments_raw, bp1, bp2, bp3, branch_points)
    ...
    pgv_seg = None
    lgv_seg = _v2_pick_unused_terminal_path(
        candidates, [sv_candidate, smv_candidate], nodes)
    comp_type = "LGV" if lgv_seg is not None else None
    print(
        "    V2 compensation: SMV is terminal endpoint branch; "
        "remaining visceral side branch is labelled LGV"
    )
...
if __name__ == '__main__':
    ...
    if args.post_tips in ("1", "true"):
        ...
        post_tips_arg = None
    segment_vessels(args.stl_path, post_tips=post_tips_arg)
