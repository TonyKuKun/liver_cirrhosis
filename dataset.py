...
class PortalVeinDataset(Dataset):
    ...
    def __init__(self, root_dir: str, n_points: int = DEFAULT_N_POINTS,
                 ...
                 include_00_prefix_samples: bool = True):
        super().__init__()
        self.root_dir = root_dir
        self.n_points = n_points
        self.label_key = label_key
        self.verbose = verbose
        self.include_00_prefix_samples = bool(include_00_prefix_samples)
        self.excluded_00_prefix_names = []
        ...
        for name in sorted(os.listdir(root_dir)):
            ...
            if not os.path.isdir(pdir):
                continue
            if not self.include_00_prefix_samples and name.startswith('00'):
                self.excluded_00_prefix_names.append(name)
                continue
        ...
        if verbose:
            ...
            if not self.include_00_prefix_samples:
                print(f"[Dataset] Excluded {len(self.excluded_00_prefix_names)} "
                      "00-prefix sample folders.")
